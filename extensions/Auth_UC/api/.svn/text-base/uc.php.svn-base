...
define('API_SYNLOGIN', 0);			//同步登录 API 接口开关
define('API_SYNLOGOUT', 0);			//同步登出 API 接口开关
...
if($action == 'test') {
	...
	if($member = $db->fetch_array($query)) {
		...
		dsetcookie('sso_eoe_auth', authcode($member['uid']."\t".$member['username'], 'ENCODE'), 86400 * 365);
	}
//---------------------------------------修改了------------------------------
	...
	//同步登出 API 接口
	header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
	dsetcookie('sso_eoe_auth', '', -86400 * 365);
